...
def update_conf(param, new_val):
    ...
    """
    #Set observed nodes
    path, port = rtctree.path.parse_path("/localhost/Rep1.rtc")
    t = rtctree.tree.RTCTree(servers='localhost', paths=path,dynamic=True) #servers='localhost:2809'servers='localhost', paths=path,dynamic=True
    arg = ["/localhost/Rep1.rtc", "set", param, str(new_val)]
    rtconf.main(argv=arg,tree=t)
    e = time.clock()
    f = open('../../Rep1_update_conf_time.txt', 'a')
    f.write(str(param) + "," + str(e-s)+'\n')
    f.close()
    self.bindParameter("tlin1", self._tlin1, "0")
    """
